 ...
 * </ul>
 */
public class _FixedGridLayoutManager extends RecyclerView.LayoutManager {

    private static final String TAG = _FixedGridLayoutManager.class.getSimpleName();
    ...
    private void fillGrid(int direction, int emptyLeft, int emptyTop, RecyclerView.Recycler recycler, boolean preLayout, SparseIntArray removedPositions) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            List<TVProgram> list = DataManager.getSharedInstance().getLists();
            TVProgram p = list.get(i);
...
//                if (!preLayout) {
//                    LayoutParams lp = (LayoutParams) view.getLayoutParams();
//                    lp.startRow = getGlobalRowOfPosition(nextPosition);
...
//                }
                ...
                layoutDecorated(view, leftOffset + p.leftOffset() * mDecoratedChildWidth, topOffset + p.topOffset() * mDecoratedChildHeight,
                        leftOffset + (p.leftOffset() + 1) * mDecoratedChildWidth,
                        topOffset + p.topOffset() * mDecoratedChildHeight + view.getLayoutParams().height);
...
//
//            List<TVProgram> list = DataManager.getSharedInstance().getLists();
//            TVProgram p = list.get(nextPosition);
//
//            if (view == null) {
//                /*
//                 * The Recycler will give us either a newly constructed view,
...
//                            topOffset + view.getLayoutParams().height);
//               }
//
//
//
...
//        }
    ...
    private void layoutTempChildView(View child, int rowDelta, int colDelta, View referenceView) {
        //Set the layout position to the global startRow/column difference from the reference view
...
//        return DataManager.ROWS * DataManager.COLUMES;
//    }
}
